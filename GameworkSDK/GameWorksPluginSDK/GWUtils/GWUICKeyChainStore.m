...
NSString * const GWUICKeyChainStoreErrorDomain = @"RWWERRORDomain";
static NSString *_defaultService;

@interface GWUICKeyChainStore () {
    NSMutableDictionary *itemsToUpdate;
}

@end

@implementation GWUICKeyChainStore
...
+ (NSString *)defaultService
{
    ...
    NSString  * keyStore =  @"com.youxigongchang.gameworkkey";
    return keyStore;
...
{
    _defaultService = @"com.youxigongchang.gameworkkey";
}

#pragma mark -

+ (GWUICKeyChainStore *)keyChainStore
{
    NSString  * keyStore =  @"com.youxigongchang.gameworkkey";
    return [[self alloc] initWithService:keyStore];
}

+ (GWUICKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service];
}

+ (GWUICKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    NSLog(@"read from TMCache.................key : %@",key);
    
    NSString * returnvalue = [GWUtility stringWithAESString:[NSString stringWithFormat:@"%@",[[TMCache sharedCache] objectForKey:key]]];
    return returnvalue;
...
+ (BOOL)setString:(NSString *)value forKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    NSString * returnvalue = [GWUtility AESStingWithString:value];
    
    [[TMCache sharedCache] setObject:returnvalue forKey:key];
...
+ (NSData *)dataForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    if (!key) {
        if (error) {
            *error = [NSError errorWithDomain:GWUICKeyChainStoreErrorDomain code:GWUICKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"`key` must not to be nil", nil)}];
        }
        return nil;
    }
    if (!service) {
    ...
    if (status != errSecSuccess) {
        if (status != errSecItemNotFound) {
            if (error) {
                *error = [NSError errorWithDomain:GWUICKeyChainStoreErrorDomain code:status userInfo:nil];
            }
        }
        return nil;
    }
...
- (void)setString:(NSString *)string forKey:(NSString *)key
{
    ...
    NSString * returnvalue = [GWUtility AESStingWithString:string];
    [[TMCache sharedCache] setObject:returnvalue forKey:key];
...
{
    NSArray *items = [GWUICKeyChainStore itemsForService:service accessGroup:accessGroup];
...
{
    NSArray *items = [GWUICKeyChainStore itemsForService:self.service accessGroup:self.accessGroup];
