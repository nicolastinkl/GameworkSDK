...
void gwbyteSwap(byte *b1, byte *b2)
{
    byte b = *b1;
    *b1 = *b2;
    *b2 = b;
...
{    //    √‹‘ø¿©’π
    ...
    for(int roundIndex = 1; roundIndex < N_ROUND; ++roundIndex)
    {
        ...
        gwbyteSwap(&rotWord[0], &rotWord[1]);
        gwbyteSwap(&rotWord[1], &rotWord[2]);
        gwbyteSwap(&rotWord[2], &rotWord[3]);
        for(int i = 0; i < 4; ++i)
        {
...
{    //    ––±‰ªª
    ...
    gwbyteSwap(&state[4], &state[5]);
    gwbyteSwap(&state[5], &state[6]);
    gwbyteSwap(&state[6], &state[7]);
    //stateµ⁄»˝––—≠ª∑◊Û“∆¡Ω∏ˆ◊÷Ω⁄
    gwbyteSwap(&state[8], &state[10]);
    gwbyteSwap(&state[9], &state[11]);
    //stateµ⁄»˝––—≠ª∑◊Û“∆»˝∏ˆ◊÷Ω⁄
    gwbyteSwap(&state[14], &state[15]);
    gwbyteSwap(&state[13], &state[14]);
    gwbyteSwap(&state[12], &state[13]);
}
